...
KeyFrame::KeyFrame(double _time_stamp, int _index, const cv::Mat &_image_intensity, const pcl::PointCloud<PointType>::Ptr _cloud,const Pose6D &_position,int _bag_id)
{
...
void KeyFrame::computeWindowOrbPoint()
{
    if (USE_ORB)
    {
        ...
        extractNormalizedPoints(orb_point_2d_uv,orb_point_2d_norm,status);
        detector->compute(image_intensity, orb_window_keypoints, orb_window_descriptors);
    }
}
void filterTopNResponses(const cv::Mat& image_intensity, std::vector<cv::Point2f>& result_points, int num_features, int top_n_to_filter) {
    std::vector<cv::Point2f> points;
    cv::goodFeaturesToTrack(image_intensity, points, num_features, 0.01, 10, cv::Mat(), 3, false, 0.04);

    cv::Mat response_map;
    cv::cornerMinEigenVal(image_intensity, response_map, 3, 3);

    std::vector<std::pair<cv::Point2f, float>> point_response_pairs;
    for (const auto& point : points) {
        int x = static_cast<int>(point.x);
        int y = static_cast<int>(point.y);
        float response = response_map.at<float>(y, x); 
        point_response_pairs.emplace_back(point, response);
    }

    std::sort(point_response_pairs.begin(), point_response_pairs.end(),
              [](const std::pair<cv::Point2f, float>& a, const std::pair<cv::Point2f, float>& b) {
                  return a.second > b.second; 
              });

    for (int i = top_n_to_filter; i < static_cast<int>(point_response_pairs.size()); ++i) {
        result_points.push_back(point_response_pairs[i].first);
    }
}
void KeyFrame::computeWindowBriefPoint()
{
    if (USE_BRIEF)
    {
        ...
        extractNormalizedPoints(brief_point_2d_uv,brief_point_2d_norm,status);
...
void KeyFrame::computeSearchOrbPoint()
{
    if (USE_ORB)
    {
        ...
        std::cout << "Detected Search ORB keypoints: " << search_orb_keypoints.size() << std::endl;
        keypointConverter(search_orb_keypoints, search_orb_point_2d_uv);
        std::cout << "Detected  search_orb_point_2d_uv keypoints1: " << search_orb_point_2d_uv.size() << std::endl;
        extractNormalizedPoints(search_orb_point_2d_uv,search_orb_point_2d_norm,status);
  
        detector->compute(image_intensity, search_orb_keypoints, search_orb_descriptors);
        std::cout << "Detected  search_orb_point_2d_uv keypoints7: " << search_orb_point_2d_uv.size() << std::endl;
    }
}
...
void KeyFrame::computeSearchBriefPoint()
{
    if (USE_BRIEF)
    {
        ...
        extractNormalizedPoints(search_brief_point_2d_uv,search_brief_point_2d_norm,status);
...
void KeyFrame::estimatePose2D2D(const std::vector<cv::Point2f> &matched_2d_cur,
                                ...
                                cv::Mat &R, cv::Mat &t)
{
    ...
    cv::Mat inliers;
    cv::Mat essential_matrix = cv::findEssentialMat(matched_2d_cur, matched_2d_old, K, cv::RANSAC, 0.999, 1.0, inliers);

    if (essential_matrix.empty())
    {
        std::cerr << "Error: Essential matrix computation failed. Not enough inliers." << std::endl;
        return;
    }

    cv::recoverPose(essential_matrix, matched_2d_cur, matched_2d_old, K, R, t, inliers);
...
{
    const double DISTANCE_THRESHOLD = 8.0; 
    double distance = sqrt(pow(this->position.x - old_kf->position.x, 2) +
                       pow(this->position.y - old_kf->position.y, 2) +
                       pow(this->position.z - old_kf->position.z, 2));

    if (distance > DISTANCE_THRESHOLD)
    {
        std::cout << "Frames are too far apart (distance = " << distance << "m), skipping loop detection." << std::endl;
        return false;
    }

    if (this->bag_id != old_kf->bag_id)
        return false;
    ...
    std::cout << "Final Total features !!!!!!!: " << search_orb_point_2d_uv.size() << std::endl;
    ...
    if (USE_ORB)
    {
        ...
        if ((int)good_matches.size() > MIN_LOOP_FEATURE_NUM)
        {
            ...
            cv::Mat R, t;
            estimatePose2D2D(matched_2d_cur, matched_2d_old, R, t);

            if (!R.empty() && !t.empty() && distributionValidation(matched_2d_cur, matched_2d_old))
            {

                std::cout << "Final ORB matched features: " << matched_2d_cur.size() << std::endl;
    ...
    if (USE_BRIEF)
    {
        ...
        if ((int)matched_2d_cur.size() > MIN_LOOP_FEATURE_NUM)
        {
            ...
            if (!R.empty() && !t.empty() && distributionValidation(matched_2d_cur, matched_2d_old))
            {
                ...
                std::cout << "Final BRIEF matched features: " << matched_2d_cur.size() << std::endl;
                pubMatchedImages(&pub_match_img, time_stamp, true, 5, cv::Scalar(0, 255, 0), index, old_kf->index, thumbnail, old_kf->thumbnail, matched_2d_cur, matched_2d_old);
                return true;
            }
        }
                         ...
                         std::vector<uchar> &status)
{
    ...
    if (matched_2d_old_norm.size() < 6 || matched_3d.size() < 6) {
        std::cerr << "Error: Not enough points for solvePnPRansac. At least 6 points are required." << std::endl;
        return;
    }
    ...
    try {
        solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, tvec, false, 100, 1.0, 0.99, inliers);
    } catch (cv::Exception &e) {
        std::cout<< matched_2d_old_norm.size()<< "matched_3d.size() " << matched_3d.size() <<std::endl;
        std::cerr << "Exception occurred _______________: " << e.what() << std::endl;
        return;
    }
...
void KeyFrame::extractNormalizedPoints(const vector<cv::Point2f>& in_point_2d_uv, 
                                       vector<cv::Point2f>& out_point_2d_norm,
                                       vector<uchar>& out_status)
{

    assert(!in_point_2d_uv.empty());
    ...
    {
        const cv::Point2f& pt_uv = in_point_2d_uv[i];

        cv::Point2f p_2d_n;
        p_2d_n.x = (pt_uv.x - cx) / fx;
        p_2d_n.y = (pt_uv.y - cy) / fy;
