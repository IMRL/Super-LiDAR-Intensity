...
public:
    ...
    ros::Publisher pub_image_1,pub_image_2;
    ...
    std::string output_image_base;

    ImageHandler()
    {
        cloud_track.reset(new pcl::PointCloud<PointType>());
        cloud_track->resize(IMAGE_HEIGHT * IMAGE_WIDTH);
        pub_image_1  = nh.advertise<sensor_msgs::Image>("loop_detector/image_stack_1", 1);
        pub_image_2  = nh.advertise<sensor_msgs::Image>("loop_detector/image_stack_2", 1);
        frame_count = 0;

        std::string pkg_path = ros::package::getPath("diff_imaging_lidar_place_recognition");
        nh.param<std::string>("model_path", model_path, pkg_path + "/scripts/weight/super_panoramic.ts");
        nh.param<std::string>("single_frame_cloud_path", single_frame_cloud_path, pkg_path + "/output_image/single_frame/");
        nh.param<std::string>("global_accumulated_cloud_path", global_accumulated_cloud_path, pkg_path + "/output_image/accumulated_cloud_global/");
        nh.param<std::string>("local_accumulated_cloud_path", local_accumulated_cloud_path, pkg_path + "/output_image/accumulated_cloud_local/");
        nh.param<std::string>("output_image_base", output_image_base, pkg_path + "/output_image/");
    ...
    void cloud_handler(const nav_msgs::Odometry::ConstPtr &odom_msg, const livox_ros_driver2::CustomMsg::ConstPtr &livox_msg,int bagid)
    {
        ...
        for (const auto &pair : cloud_odom_queue)
        {
            ...
            *accumulated_cloud_global += *cloud_global;
            ...
            {
                savePointCloud(cloud, single_frame_cloud_path, "single_frame", frame_count);
            }
        }
        ...
        *accumulated_cloud_local = *global2local(accumulated_cloud_global, odom_first);
        ...
        {           
            ...
            std::string sparse_filename = output_image_base + "sparse_image/intensity_sparse_" + std::to_string(frame_count) + ".png";
            std::string dense_filename = output_image_base + "dense_image/intensity_dense_" + std::to_string(frame_count) + ".png";
        ...
        if (pub_image_1.getNumSubscribers() != 0 || pub_image_2.getNumSubscribers() != 0)
        {
            ...
            if (bagid ==1)
            {
                pubImage(&pub_image_1, image_visualization, livox_msg->header, "bgr8");
            }
            else
            {
                pubImage(&pub_image_2, image_visualization, livox_msg->header, "bgr8");
            }
