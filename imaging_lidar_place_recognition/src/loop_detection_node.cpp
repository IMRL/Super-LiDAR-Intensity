...
void visualizeLoopClosure_10(ros::Publisher *pub_m, ros::Publisher *pub_i, 
                          ros::Time timestamp, 
                          const std::map<int, int>& match_container, 
                          const std::map<int, int>& pose_container,
                          const pcl::PointCloud<PointType>::Ptr cloud_path)
{
    if (cloud_path->empty() || match_container.empty())
        return;
    ...
    std::vector<std::pair<int, int>> recent_matches(match_container.rbegin(), match_container.rend());
    if (recent_matches.size() > 10) {
        recent_matches.resize(10);
    }

    for (const auto& match : recent_matches)
    {
        int key_cur = match.first;
        int key_pre = match.second;

        geometry_msgs::Point p;
        int cloud_pose_id = -1;

        auto it_cur = pose_container.find(key_cur);
        if (it_cur != pose_container.end()) {
            cloud_pose_id = it_cur->second;
            p.x = cloud_path->points[cloud_pose_id].x;
            p.y = cloud_path->points[cloud_pose_id].y;
            p.z = cloud_path->points[cloud_pose_id].z;
            markerNode.points.push_back(p);
            markerEdge.points.push_back(p);
            indexArray.data.push_back(cloud_pose_id);
        }

        auto it_pre = pose_container.find(key_pre);
        if (it_pre != pose_container.end()) {
            cloud_pose_id = it_pre->second;
            p.x = cloud_path->points[cloud_pose_id].x;
            p.y = cloud_path->points[cloud_pose_id].y;
            p.z = cloud_path->points[cloud_pose_id].z;
            markerNode.points.push_back(p);
...
void path_handler(const nav_msgs::PathConstPtr& path_msg)
{
    ...
    {
        start_time = path_msg->poses.front().header.stamp.toSec();
        end_time = path_msg->poses.back().header.stamp.toSec();
    }

    total_distance = 0.0;
    if (cloud_traj->size() > 1)
    {
        for (size_t i = 1; i < cloud_traj->size(); ++i)
        {
            double dx = cloud_traj->points[i].x - cloud_traj->points[i - 1].x;
            double dy = cloud_traj->points[i].y - cloud_traj->points[i - 1].y;
            double dz = cloud_traj->points[i].z - cloud_traj->points[i - 1].z;
            total_distance += std::sqrt(dx * dx + dy * dy + dz * dz);
        }
    }

    double total_time = end_time - start_time;
    double average_speed = total_time > 0 ? total_distance / total_time : 0.0;

    int total_matches = index_match_container.size();

    ROS_INFO("\033[1;34m----> Total Matches: \033[1;32m%d\033[0m", total_matches);
    ROS_INFO("\033[1;34m----> Trajectory Length: \033[1;32m%.2f meters\033[0m", total_distance);
    ROS_INFO("\033[1;34m----> Trajectory Time: \033[1;32m%.2f seconds\033[0m", total_time);
    ROS_INFO("\033[1;34m----> Average Speed: \033[1;32m%.2f m/s\033[0m", average_speed);

    std::cout << " cloud_traj size is : " << cloud_traj->size() << std::endl;
}

void cloud_handler(const nav_msgs::Odometry::ConstPtr& odom_msg, const livox_ros_driver2::CustomMsg::ConstPtr& cloud_msg)
{
    image_handler->cloud_handler(odom_msg,cloud_msg);
    ...
    visualizeLoopClosure_10(&pub_marker_10, &pub_index_10, cloud_msg->header.stamp, index_match_container, index_poseindex_container, cloud_traj);
    ...
    for (int i = 0; i < IMAGE_HEIGHT; ++i)
        for (int j = 0; j < IMAGE_WIDTH; ++j)
            if (j < IMAGE_CROP || j > IMAGE_WIDTH - IMAGE_CROP)
                MASK.at<uchar>(i,j) = 0;
    message_filters::Subscriber<nav_msgs::Odometry> subOdometry(n, ODOM_TOPIC, 1);
    message_filters::Subscriber<livox_ros_driver2::CustomMsg> subLaserCloud(n, CLOUD_TOPIC, 1);

    typedef message_filters::sync_policies::ApproximateTime<nav_msgs::Odometry, livox_ros_driver2::CustomMsg> MySyncPolicy;
    message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), subOdometry, subLaserCloud);
    sync.registerCallback(boost::bind(&cloud_handler, _1, _2));
