...
ros::Publisher pubIntensityImage, pubLeftImage, pubRightImage, pubBackImage,pubDepthImage,pubIntensityImageDense,pubDepthImageDense,pubLaneImage;
...
cv::Mat generateImage(const pcl::PointCloud<PointType>::Ptr& cloud, int width, int height, bool use_intensity)
{
    ...
    const float vertical_fov_min = -7.0f * M_PI / 180.0f;  
    const float vertical_fov_max = 52.0f * M_PI / 180.0f;  
...
std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>> nms_wrapper(
    ...
    std::cout << "Inside nms_wrapper:" << std::endl;
    std::cout << "Batch size: " << batch_proposals.size(0) << std::endl;
...
std::vector<Lane> proposals_to_pred(const torch::Tensor& proposals) {
    ...
    for (int i = 0; i < proposals.size(0); ++i) {
        ...
        std::cout << "End: " << end << ", Anchor_ys_device size: " << anchor_ys_device.size(0)
        ...
        // std::cout << "Slice size: " << slice.sizes() << ", Content: " << slice << std::endl;
        ...
        lanes.push_back({
            points,
            lane[3].item<double>(),  // start_x
            lane[2].item<double>(),  // start_y
            lane[1].item<double>()   // conf
        });
    }
...
std::vector<std::vector<Lane>> get_lanes(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>>& proposals_list) {
    ...
    std::cout << "Inside get_lanes:" << std::endl;
    std::cout << "Proposals list size: " << proposals_list.size() << std::endl;

    for (size_t i = 0; i < proposals_list.size(); ++i) {
        const auto& proposals = std::get<0>(proposals_list[i]);

        std::cout << "  Proposals shape: " << proposals.sizes() << std::endl;

        if (proposals.size(0) == 0) {
            std::cout << "  No proposals found for this image." << std::endl;
        ...
        std::cout << "  Number of lanes decoded: " << lanes.size() << std::endl;

        for (size_t j = 0; j < lanes.size(); ++j) {
            std::cout << "    Lane " << j << " contains " << lanes[j].points.size() << " points." << std::endl;
        }
...
cv::Mat run_lane_detect_model(const cv::Mat& intensity_image) {
    std::cout << "Input image size: " << intensity_image.size() << std::endl;
    std::cout << "Input image channels: " << intensity_image.channels() << std::endl;
    ...
    auto dict = input_dict.toGenericDict();
    ...
    std::cout << "NMS completed" << std::endl;


    std::cout << "Decoding proposals into lanes..." << std::endl;
    auto all_lanes = get_lanes(proposals_list);
    std::cout << "Decoded lanes size: " << all_lanes.size() << std::endl;
...
void cloud_handler(const nav_msgs::Odometry::ConstPtr &odom_msg, const livox_ros_driver2::CustomMsg::ConstPtr &livox_msg)
{
    ...
    *accumulated_cloud_local = *global2local(accumulated_cloud_global, odom_first);
    ...
    if (saveImg && frameCount % 10 == 0)
    {
        std::string intensity_save_path_front = imagesavePath + "intensity_front_n21/front_" + std::to_string(frameCount) + ".png";
...
int main(int argc, char** argv)
{
    ...
    model_path = pkg_path + "/model/four_view_afm_s2d_epoch_9.ts";
    imagesavePath = pkg_path + "/image_save/";
    lane_model_path = pkg_path + "/model/laneatt/test.ts";

    load_model();
    load_lane_detect_model();
    nhPrivate.getParam("lidar_topic", lidar_topic);
    nhPrivate.getParam("odom_topic", odom_topic);
    nhPrivate.getParam("frame_window", frame_window);
    nhPrivate.getParam("image_process", image_process);
