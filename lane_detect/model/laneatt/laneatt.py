...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        '''
        self.test_param = torch.jit.Attribute(
            self.cfg.test_parameters,  # 确保类型一致
            Tuple[any, any],  # 显式声明类型
        )
        '''

        #self.test_param = self.cfg.test_parameters

        self.conf_threshold = self.cfg.test_parameters.conf_threshold
        self.nms_thres = self.cfg.test_parameters.nms_thres
        self.nms_topk = self.cfg.test_parameters.nms_topk
    ...
    def forward(self, x,): # **kwargs
        ...
        non_diag_inds = torch.nonzero(attention_matrix == 0.)
        attention_matrix[:] = 0
        ...
        output_shape = (batch_size, num_anchors, 5 + self.n_offsets)
        reg_proposals = torch.zeros(output_shape, device=x.device)
        ...
        return reg_proposals, attention_matrix, self.anchors#proposals_list.float()  #output #


    def nms(self, batch_proposals, batch_attention_matrix, nms_thres, nms_topk, conf_threshold):

        proposals_list: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]] = []
        for proposals, attention_matrix in zip(batch_proposals, batch_attention_matrix):
            ...
            with torch.no_grad():
                scores = self.softmax(proposals[:, :2])[:, 1]
                ...
                if proposals.shape[0] == 0:
                    empty_tensor = torch.empty(0, device=proposals.device)
                    proposals_list.append((proposals[[]], self.anchors[[]], attention_matrix[[]], empty_tensor))
                    continue
    ...
    def match_proposals_with_targets(self, proposals, targets, t_pos=15., t_neg=20.):
        ...
        valid_offsets_mask = targets.new_zeros(targets.shape, device=device) 
        all_indices = torch.arange(valid_offsets_mask.shape[0], dtype=torch.long, device=device)  
    ...
    def loss(self, output, batch, cls_loss_weight=10):
        device = output['proposals_list'][0][0].device  
    ...
    def cut_anchor_features(self, features):
        ...
        n_proposals = self.anchors.shape[0] 
    ...
    def get_lanes(self, output, as_lanes=True):
        ...
        for proposals, _, _, _ in proposals_list:
            proposals[:, :2] = self.softmax(proposals[:, :2])
